...
├── log/
│   ├── server/                 ← Game server output logs
...
### `ServerDescription.json` (root of serverfiles/)
...
```bash
# Re-run Wine prefix setup with vcrun2022
sudo -u windrose bash -c "WINEPREFIX=/home/windrose/.wine winetricks vcrun2022"
```
...
```
Then restart the server so it rewrites `ServerDescription.json`.

### UE5 "display" errors
Ensure `xvfb-run` and `Xvfb` are functional:
```bash
xvfb-run -a wine64 --version
```
...
sudo -u windrose /home/windrose/windroseserver details
# Or directly:
cat /home/windrose/serverfiles/ServerDescription.json
```

### Check logs
```bash
sudo -u windrose /home/windrose/windroseserver logs
# Script logs:
